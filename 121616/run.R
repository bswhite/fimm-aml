...
nplr.calc.ic50 <- function(dose, viability, file, patient.id, inhibitor, apply.constraints=FALSE) {
...
calc.ic50 <- function(dose, viability, file) {
  ...
  # plot IC50
  png(paste(output.path, file, sep="/"))
  x <- seq(min(mat_tbl$logconc),max(mat_tbl$logconc), length=100)
  y <- predict(nls_result_ic50, data.frame(logconc=x))
...
if(debug.case.with.ic50.outside.range) {
  ...
  r <- nplr.calc.ic50(df$well_concentration, df$normalized_viability, "trouble", pid, inh, apply.constraints=TRUE)

  r <- nplr.calc.ic50(df$well_concentration, df$normalized_viability, "trouble", pid, inh, apply.constraints=FALSE)
...
tbl2 <- foreach(i=1:n.iters, .combine=rbind) %dopar% {
  ...
  if(use.nplr) {
    ...
    row <- r[["tbl"]]
    results2[[i]] <- list(ID=row$ID, result=r[["res"]])
  } else {
...
## do -> dopar
results2.refit <- list()
tbl2.refit <- foreach(i=1:n.iters, .combine=rbind) %do% {
  # for(i in 1:nrow(patient.drug.tbl)) {
  if( ( i %% 10 ) == 0 ) { cat(paste0("Iter ", i, "\n")) }
